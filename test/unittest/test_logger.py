...
class TestLoggerConfig:
    ...
    def test_initialize_normal_mode(self):
        ...
        LoggerConfig.initialize(
            debug=False, log_to_file=False, log_to_console=False, colored_output=False, verbose=False
        )
    ...
    def test_initialize_verbose_mode(self):
        ...
        """
        LoggerConfig.initialize(
            debug=False, log_to_file=False, log_to_console=False, colored_output=False, verbose=True
        )
    ...
    def test_initialize_debug_mode(self):
        ...
        """
        LoggerConfig.initialize(
            debug=True, log_to_file=False, log_to_console=False, colored_output=False, verbose=False
        )
    ...
    def test_set_level_all_modules(self):
        ...
        """
        LoggerConfig.initialize(debug=False, log_to_file=True, log_to_console=True, colored_output=False, verbose=False)
    ...
    def test_enable_disable_debug(self):
        ...
        """
        LoggerConfig.initialize(
            debug=False, log_to_file=False, log_to_console=True, colored_output=False, verbose=False
        )
    ...
    def test_initialization_idempotent(self):
        """
        Scenario: Re-initialize with different settings

        Given LoggerConfig is already initialized with debug=False
        When initialize() is called again with debug=True
        Then the handler levels should be updated to reflect new settings
        """
        ...
        # Should now have the updated settings
        assert LoggerConfig._console_level == logging.DEBUG

    def test_initialize_with_tilde_log_dir(self, monkeypatch):
        """
        Scenario: Initialize logging with tilde in log_dir

        Given log_dir parameter with tilde path
        When LoggerConfig.initialize() is called
        Then tilde should be expanded
        And log directory should be created at expanded location
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            # Mock os.path.expanduser which is what Path.expanduser() uses
            def mock_expanduser(path):
                if path and "~" in path:
                    return path.replace("~", tmpdir, 1)
                return path

            monkeypatch.setattr("os.path.expanduser", mock_expanduser)

            # Initialize with tilde path
            LoggerConfig.initialize(
                debug=False,
                log_to_file=True,
                log_to_console=False,
    ...
    def test_initialize_with_expanded_log_dir(self):
        """
        Scenario: Initialize logging with already expanded log_dir

        Given log_dir parameter with already expanded path
        When LoggerConfig.initialize() is called
        Then it should work correctly without double expansion
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            expanded_path = Path(tmpdir) / "already_expanded" / "logs"

            # Initialize with expanded path
            LoggerConfig.initialize(
                debug=False,
                log_to_file=True,
                log_to_console=False,
                log_dir=str(expanded_path),
                colored_output=False,
                verbose=False,
            )

            # Should use the path as-is
            assert LoggerConfig._log_dir == expanded_path
    ...
    def test_path_expanduser_behavior(self, monkeypatch):
        """
        Scenario: Test Path.expanduser() behavior
