...
"""Step: Settings > Autosave Settings — open and close the dialog."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("SETTINGS MENU - AUTOSAVE SETTINGS")
    ensure_test_scene_loaded(ctx)
    ctx.log.action("Opening autosave settings dialog")
    close_open_dialog(ctx)
    trigger_action(ctx, "autosave_settings")
    wait(ctx, SHORT_DELAY_MS, "Wait for autosave settings dialog to appear and close")
    ctx.log.success("Autosave settings dialog opened and closed")
