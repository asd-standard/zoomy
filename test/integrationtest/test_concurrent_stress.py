...
class TestConverterRunnerStress:
    ...
    def test_many_concurrent_conversions(self, sample_image_batch, tmp_path):
        ...
        _done, not_done = wait([f for f, _ in futures], timeout=300)  # 5 minutes
        ...
        print(f"Stress test: {success_count}/{len(futures)} successful ({success_rate:.1%}) in {elapsed:.1f}s")
    ...
    def test_concurrent_submission_with_many_threads(self, sample_image_batch, tmp_path):
        ...
        _done, not_done = wait(futures, timeout=300)
    ...
    def test_mixed_conversion_and_tiling_workloads(self, sample_image_batch, tmp_path):
        ...
        if valid_ppm_files:
            ...
            for i, ppm_file in enumerate(valid_ppm_files):
                ...
                future = tilerrunner.submit_tiling(ppm_file, media_id=media_id, tilesize=256)
                tiling_futures.append(future)

            # Wait for tiling
            _done, not_done = wait(tiling_futures, timeout=300)
            ...
            print(f"Mixed workload: {success_count}/{len(valid_ppm_files)} tiling operations successful")
            ...
            try:
                ...
                for i in range(2):
                    ...
                    if i < len(sample_image_batch):
                        ...
                        conv_future = converterrunner.submit_vips_conversion(infile, outfile)
                        ...
                        if conv_error is None and os.path.exists(outfile):
                            ...
                            tile_future = tilerrunner.submit_tiling(outfile, media_id=media_id, tilesize=256)
...
class TestResourceCleanupStress:
    ...
    def test_resource_cleanup_under_load(self, sample_image_batch, tmp_path):
        ...
        for cycle in range(cycles):
            ...
            # Wait for completion
            _done, _not_done = wait(futures, timeout=180)
    ...
    def test_repeated_operations_with_cleanup(self, sample_image_batch, tmp_path):
        ...
        for cycle in range(cycles):
            ...
            # Wait with timeout
            done, _not_done = wait(futures, timeout=120)
