...
"""Step: Settings > Zoom Settings — open and close the dialog."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("SETTINGS MENU - ZOOM SETTINGS")
    ensure_test_scene_loaded(ctx)
    ctx.log.action("Opening zoom settings dialog")
    close_open_dialog(ctx)
    trigger_action(ctx, "zoom_settings")
    wait(ctx, SHORT_DELAY_MS, "Wait for zoom settings dialog to appear and close")
    ctx.log.success("Zoom settings dialog opened and closed")
