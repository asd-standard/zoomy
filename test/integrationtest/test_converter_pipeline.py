...
class TestConverterThreading:
    ...
    Converters extend Thread and can run in background threads,
    enabling non-blocking conversion operations.
    """
    ...
    def test_converter_runs_in_thread(self, sample_images, tmp_path):
        ...
        When start() is called instead of run()
        Then conversion executes in a separate thread
        ...
        converter = VipsConverter(infile, outfile)
        converter.start()  # Start in background thread

        # Main thread continues
        assert converter.is_alive() or converter.progress == 1.0
    ...
    @pytest.mark.skipif(not is_pyvips_available(), reason="pyvips not available")
    def test_multiple_converters_run_concurrently(self, sample_images, tmp_path):
        """
        Scenario: Multiple converters run concurrently

        Given multiple converter instances
        When all are started concurrently
        ...
        for i in range(3):
            ...
            converters.append(VipsConverter(infile, outfile))
...
class TestConcurrentConversionOperations:
    ...
    Multiple conversion operations may run simultaneously. The system
    must handle concurrent disk access and resource usage correctly.
    """
    ...
    def test_concurrent_conversions_complete_independently(
        ...
        When conversions run in parallel using ThreadPoolExecutor
        Then each conversion completes independently
        And all outputs are valid
        """
        files_to_convert = [
            (sample_images['png'], tmp_path / "out1.ppm"),
            (sample_images['jpg'], tmp_path / "out2.ppm"),
            (sample_images['tiff'], tmp_path / "out3.ppm"),
        ]

        def convert_file(args):
            infile, outfile = args
            converter = VipsConverter(infile, str(outfile))
            converter.run()
            return converter.error

        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(convert_file, args) for args in files_to_convert]
        ...
        for _, outfile in files_to_convert:
            assert os.path.exists(outfile)
    ...
    def test_concurrent_conversion_and_tiling(
        ...
        When full pipelines run concurrently
        ...
        def convert_and_tile(index, infile):
            try:
                ppm_file = str(tmp_path / f"converted_{index}.ppm")
                media_id = f"concurrent_pipeline_{index}"
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                executor.submit(convert_and_tile, i, sample_images['png']): i
                for i in range(3)
            }
