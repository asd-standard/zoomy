...
Examples:
  ...
  %(prog)s                        # Clean files older than 3 days
    ...
    parser.add_argument(
        "--age", type=int, default=3, metavar="DAYS", help="Maximum age in days before deletion (default: 3)"
    )

    parser.add_argument("--dry-run", action="store_true", help="Show what would be deleted without actually deleting")

    parser.add_argument("--stats", action="store_true", help="Show tilestore statistics only (no cleanup)")
    ...
    cleanup_stats = TileStore.cleanup_old_tiles(max_age_days=args.age, dry_run=args.dry_run)
