...
"""Step: Help > About — open and close the About dialog."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("HELP MENU - ABOUT")
    ctx.log.action("Showing About dialog")
    close_open_dialog(ctx)
    trigger_action(ctx, "about")
    wait(ctx, SHORT_DELAY_MS, "Wait for About dialog to appear and close")
    ctx.log.success("About dialog opened and closed")
