...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("FILE MENU - QUIT")
    ctx.log.action("Triggering quit (confirm dialog will appear)")
    close_open_dialog(ctx)
    trigger_action(ctx, "quit")
    wait(ctx, SHORT_DELAY_MS, "Wait for confirm quit dialog to appear and close")
    ctx.log.success("Quit test completed (dialog closed, app not exited)")
