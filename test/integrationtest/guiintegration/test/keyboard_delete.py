...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("KEYBOARD - DELETE MEDIA")
    ...
    ctx.log.action("Press DELETE to remove selected image")
    simulate_key(ctx, Qt.Key_Delete)
    wait(ctx, DEFAULT_DELAY_MS, "Observe: One image DELETED")
    ctx.log.action("Reloading test scene")
    ctx.scene_loaded = False
    ensure_test_scene_loaded(ctx)
    ctx.log.success("Delete completed, scene reloaded")
