...
"""Step: Right-click image to open modify image dialog."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("MOUSE RIGHT-CLICK - IMAGE MODIFICATION DIALOG")
    ensure_test_scene_loaded(ctx)
    zui = ctx.window.zui
    center = QPoint(zui.width() // 2, zui.height() // 2)
    ctx.log.action("Right-click on image to open modify dialog")
    close_open_dialog(ctx)
    simulate_mouse_click(ctx, center, Qt.RightButton)
    wait(ctx, SHORT_DELAY_MS, "Wait for modify image dialog to appear and close")
    ctx.log.success("Image modification dialog opened and closed")
