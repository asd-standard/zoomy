...
"""Step: Mouse left-click select on test scene."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("MOUSE INTERACTIONS - LEFT CLICK SELECT")
    ensure_test_scene_loaded(ctx)
    zui = ctx.window.zui
    center = QPoint(zui.width() // 2, zui.height() // 2)
    corner = QPoint(50, 50)
    ctx.log.action("Left-click CENTER to select an image")
    simulate_mouse_click(ctx, center)
    wait(ctx, DEFAULT_DELAY_MS, "Observe: Image SELECTED (highlighted)")
    ctx.log.action("Left-click CORNER to deselect")
    simulate_mouse_click(ctx, corner)
    wait(ctx, DEFAULT_DELAY_MS, "Observe: Image DESELECTED")
    ctx.log.success("Click select test completed")
