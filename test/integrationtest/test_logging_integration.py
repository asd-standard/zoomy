...
class TestConfigurationFile:
    ...
    def test_config_file_logging_settings(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # INFO should appear in verbose mode (strip ANSI codes for comparison)
            import re

            console_output_no_ansi = re.sub(r"\x1b\[[0-9;]*m", "", console_output)
            assert "INFO - should appear (verbose mode)" in console_output_no_ansi
