...
class TestConverterRunnerIntegration:
    ...
    def test_converterrunner_thread_safety_during_concurrent_submissions(self, sample_images, tmp_path):
        ...
        def submit_conversion_thread(index):
            ...
            try:
                ...
                handle = converterrunner.submit_vips_conversion(sample_images["png"], outfile)
                with results_lock:
    ...
    @pytest.mark.skipif(not is_pyvips_available(), reason="pyvips not available")
    def test_converterrunner_default_context_is_spawn(self):
        """
        ...
        # Creating threads should NOT cause context switch (already spawn)
    ...
    def test_conversion_future_completion(self, sample_images, tmp_path):
        ...
        future = converterrunner.submit_vips_conversion(sample_images["png"], outfile)
        ...
        try:
            ...
            assert error is not None or os.path.exists(outfile), "Either error should be set or file should exist"
    ...
    def test_converterrunner_shutdown_with_pending_tasks(self, sample_images, tmp_path):
        ...
        for handle in handles:
            try:
                ...
                handle._future.result(timeout=0.1)
            except Exception:
                cancelled_count += 1
